...
local function normalize(x: number, y: number): (number, number)
  local l = length(x, y)
  if l > 0.00001 then return x / l, y / l end
  return 1.0, 0.0
end
...
function getOffset(self: RippleEffect, distance: number, pathLength: number, x: number, y: number, nx: number, ny: number): (number, number)
  ...
  local fade = 1.0
  if pathLength > 0.1 then
    local ratio = distance / pathLength
    -- Start fade
    if self.startFade > 0 then
      local sf = self.startFade * 0.01
      if sf > 0 and ratio < sf then fade = fade * ((math.abs(ratio) / sf) ^ 2) end
    end
    -- End fade
    if self.endFade > 0 then
      local ef = self.endFade * 0.01
      if ef > 0 and ratio > (1.0 - ef) then fade = fade * ((math.abs(1.0 - ratio) / ef) ^ 2) end
    end
  end
...
function update(self: RippleEffect, path: PathData): PathData
  ...
  for i = 1, #path do
    ...
    elseif cmd.type == 'lineTo' then
      ...
      local dx, dy = pt.x - startX, pt.y - startY
      local len = length(dx, dy)
      ...
      if len < 0.01 then
        -- Skip emitting zero-length segments to prevent C++ stroke crashing
      else
        for s = 1, steps do
          local t = s / steps
          local cl = len * t
          local bx, by = startX + dx * t, startY + dy * t
          local offX, offY = self:getOffset(dist + cl, currentPathLen, bx, by, px2, py2)
          local nxPos, nyPos = bx + offX, by + offY
          if math.abs(nxPos - lastX) > 0.01 or math.abs(nyPos - lastY) > 0.01 then
            outPath:lineTo(Vector.xy(nxPos, nyPos))
            lastX, lastY = nxPos, nyPos
          end
        end
      end
    ...
    elseif cmd.type == 'cubicTo' then
      ...
      if approxLen < 0.01 then
        ...
        local nxPos, nyPos = x + offX, y + offY
        if math.abs(nxPos - lastX) > 0.01 or math.abs(nyPos - lastY) > 0.01 then
          outPath:lineTo(Vector.xy(nxPos, nyPos))
          lastX, lastY = nxPos, nyPos
        end

--Fix odd particle behavior at the ends of the path emitter when force is applied. 
...
type ParticleSystemNode = {
  ...
  -- Path emitter
  -- ---------------------------------------------------------------------------
  -- Rive scripting can only read a path's geometry from an Artboard, so the
  -- emitter shape is authored on its own artboard (draw a single path/shape in
  -- it) and wired in through this input. At init we instance that artboard and
  -- bake its geometry into `pathEmitterPath` via Artboard:addToPath, then use
  -- PathData:measure() to sample points/tangents along it.
  usePathEmitter: Input<boolean>,
  pathArtboard: Input<Artboard>,
  -- Name of the path node inside the emitter artboard to read geometry from
  -- (e.g. "path"). If empty, falls back to baking the whole artboard.
  pathNodeName: Input<string>,
  -- How particles leave the path:
  --   0 = Direction    -> use the normal angle/radial emission (same as before)
  --   1 = Perpendicular -> along the path NORMAL, randomly to either side
  --   2 = Along path    -> along the path TANGENT, one direction (negate speed
  --                        to flip it)
  pathEmitMode: Input<number>,
  -- Uniform scale + offset applied to the baked emitter path so it can be
  -- positioned/sized within the particle system's local space.
  pathScale: Input<number>,
  pathOffsetX: Input<number>,
  pathOffsetY: Input<number>,

  -- Path force: drives particles ALONG the path tangent and fades with the
  -- distance from the path. `pathForce` is a signed intensity (negative pushes
  -- the other way along the tangent). `pathForceRange` is the distance (in
  -- local units) at which the force has faded to zero.
  pathForce: Input<number>,
  pathForceRange: Input<number>,

  -- Baked emitter geometry + its measurer (rebuilt at init).
  pathEmitterPath: Path,
  pathMeasure: PathMeasure?,
  pathLength: number,
  -- Precomputed sample points along the path used to find the nearest point for
  -- the path force (avoids re-measuring every particle every frame).
  pathSamplesX: { number },
  pathSamplesY: { number },
  pathTangentsX: { number },
  pathTangentsY: { number },
...
local function grad3D(hash: number, dx: number, dy: number, dz: number): number
  ...
  local res = ((h % 2) == 0 and u or -u)
    + ((mfloor(h / 2) % 2) == 0 and v or -v)
  return res
end
...
-- Number of points sampled along the emitter path for the nearest-point search
-- used by the path force. Higher = more accurate force direction, more memory.
local PATH_FORCE_SAMPLES = 200

-- Find the first path-bearing node below the named emitter node. Path geometry
-- is commonly nested under a group or shape, rather than on the named node.
local function findFirstPathData(node: NodeData): (PathData?, Mat2D?)
  local pathData = (node :: any):asPath()
  if pathData then
    return pathData, (node :: any).worldTransform
  end

  for _, child in ipairs((node :: any).children) do
    local childPathData, childTransform = findFirstPathData(child)
    if childPathData then
      return childPathData, childTransform
    end
  end
...
-- Bake the emitter artboard's geometry into `pathEmitterPath` (scaled/offset),
-- build a PathMeasure, and precompute a sample table for the path force.
-- Safe to call when there's no path artboard wired up (it just clears state).
local function buildPathEmitter(self: ParticleSystemNode)
  local dst = self.pathEmitterPath
  dst:reset()
  self.pathMeasure = nil
  self.pathLength = 0

  if not self.usePathEmitter then
    return
  end

  -- Instance the emitter artboard and bake its geometry (transformed by our
  -- scale/offset) into the destination path.
  local emitterInstance = self.pathArtboard:instance()
  if not emitterInstance then
    return
  end
  emitterInstance:advance(0)

  local s = self.pathScale
  if s == 0 then
    s = 1
  end
  local transform =
    Mat2D.withScaleAndTranslation(s, s, self.pathOffsetX, self.pathOffsetY)

  -- Preferred: resolve the actual path geometry below the named node. The
  -- selected node may be a group/shape, with its drawable path nested inside.
  local built = false
  local nodeName = self.pathNodeName
  if nodeName ~= '' then
    local node = emitterInstance:node(nodeName)
    if node then
      local pathData, worldTransform = findFirstPathData(node)
      if pathData and worldTransform then
        dst:add(pathData, transform * worldTransform)
        built = true
      end
    end
  end

  -- Fallback: bake the whole artboard's geometry.
  if not built then
    emitterInstance:addToPath(dst, transform)
  end

  local measure = dst:measure()
  local len = measure.length
  print(
    '[ParticleSystem] path emitter built=',
    built,
    'commands=',
    #dst,
    'length=',
  ...
  self.pathMeasure = measure
  self.pathLength = len
 
  -- Precompute evenly spaced samples (position + tangent) for the force lookup.
  local sx = self.pathSamplesX
  local sy = self.pathSamplesY
  local tx = self.pathTangentsX
  local ty = self.pathTangentsY
  for i = 0, PATH_FORCE_SAMPLES - 1 do
    local d = (i / (PATH_FORCE_SAMPLES - 1)) * len
    local pos, tan = measure:positionAndTangent(d)
    local idx = i + 1
    sx[idx] = pos.x
    sy[idx] = pos.y
    tx[idx] = tan.x
  ...
  -- Trim any stale trailing samples from a previous (longer) build.
  for i = PATH_FORCE_SAMPLES + 1, #sx do
    sx[i] = nil
    sy[i] = nil
    tx[i] = nil
    ty[i] = nil
...
local function spawn(sys: ParticleSystemNode, p: Particle)
  ...
  -- ---------------------------------------------------------------------------
  -- Path emitter: spawn at a random point along the baked path, and (for the
  -- perpendicular/along-path modes) derive velocity from the local normal /
  -- tangent instead of the rectangular emitter logic below.
  -- ---------------------------------------------------------------------------
  if sys.usePathEmitter and sys.pathMeasure and sys.pathLength > 0 then
    local measure = sys.pathMeasure
    local d = mrandom() * sys.pathLength
    local pos, tan = measure:positionAndTangent(d)
    p.x = pos.x
    p.y = pos.y

    local mode = sys.pathEmitMode
    local sp = randomRange(sys.speed, sys.speedVar)

    if mode == 1 then
      -- Perpendicular: emit along the normal, randomly to either side.
      -- Normal of a 2D tangent (tx, ty) is (-ty, tx).
      local nxn = -tan.y
      local nyn = tan.x
      local side = (mrandom() < 0.5) and 1 or -1
      p.vx = nxn * sp * side
      p.vy = nyn * sp * side
    elseif mode == 2 then
      -- Along path: emit along the tangent in a single direction. A negative
      -- `speed` naturally reverses it because we don't flip the sign here.
      p.vx = tan.x * sp
      p.vy = tan.y * sp
    else
      -- mode 0 (Direction): keep the standard angle / radial emission below,
      -- but from the path position we just chose.
    ...
    -- Common per-particle setup that follows (gravity/wind/noise/rotation/
    -- trail/instance) is shared, so jump past the rectangular emitter block.
    ...
    if sys.rotationMaxSpeed > sys.rotationMinSpeed then
      rSpeed = sys.rotationMinSpeed
        + mrandom() * (sys.rotationMaxSpeed - sys.rotationMinSpeed)
    end
    local dir = 1
...
local function init(self: ParticleSystemNode, context: Context): boolean
  ...
  -- Bake the emitter path (if a path artboard is wired up and enabled).
  buildPathEmitter(self)
...
local function advance(self: ParticleSystemNode, seconds: number): boolean
  ...
  -- Cache path-force sample tables locally for the hot update loop.
  local psx = self.pathSamplesX
  local psy = self.pathSamplesY
  local ptx = self.pathTangentsX
  local pty = self.pathTangentsY
  local pathSampleCount = #psx
  ...
  while i <= count do
    ...
    if p.life >= p.maxLife then
      ...
      -- Path force: push the particle along the path's tangent at the nearest
      -- point, fading with distance from the path. Find the nearest precomputed
      -- sample, then apply the tangent scaled by a linear distance falloff.
      if
        self.usePathEmitter
        and self.pathForce ~= 0
        and self.pathLength > 0
        and pathSampleCount > 0
      then
        local bestDistSq = math.huge
        local bestIdx = 1
        for si = 1, pathSampleCount do
          local ddx = p.x - psx[si]
          local ddy = p.y - psy[si]
        ...
        local range = self.pathForceRange
        local dist = math.sqrt(bestDistSq)
        -- Linear falloff: full strength on the path, 0 at (and beyond) range.
        local falloff = 1
        if range > 0 then
          falloff = 1 - dist / range
        end
        if falloff > 0 then
          -- Tangent-driven push; mass resists the force like the wind field.
          local f = self.pathForce * falloff * invMass
          turbX = turbX + ptx[bestIdx] * f
          turbY = turbY + pty[bestIdx] * f
        end
      end
      ...
      if p.instance and p.instance.data and p.instance.data.pop then
        ...
        if
          (p.maxLife - p.life) <= self.popDuration
          and p.life >= (p.maxLife * 0.3)
        then
          shouldPop = true
        end
        ...
        if not shouldPop and self.popOutside then
          if
            p.x < 0
            or p.x > self.emitWidth
            or p.y < 0
            or p.y > self.emitHeight
          then
            shouldPop = true
          end
        end
...
local function draw(self: ParticleSystemNode, renderer: Renderer)
  ...
  if self.drawEmitter then
    if self.usePathEmitter and self.pathLength > 0 then
      -- Draw the baked emitter path outline (already scaled/offset).
      renderer:drawPath(self.pathEmitterPath, self.emitterPaint)
    else
...
return function(): Node<ParticleSystemNode>
  return {
    -- Path emitter
    usePathEmitter = false,
    pathArtboard = late(),
    pathNodeName = 'path',
    pathEmitMode = 0, -- 0=Direction, 1=Perpendicular, 2=Along path
    pathScale = 1,
    pathOffsetX = 0,
    pathOffsetY = 0,
    pathForce = 0,
    pathForceRange = 100,
    pathEmitterPath = late(),
    pathMeasure = nil,
    pathLength = 0,
    pathSamplesX = {},
    pathSamplesY = {},

...
type ParticleSystemNode = {
    ...
    groupPointsCreated: boolean, -- Flag to only spawn group points once
...
local function spawnParticle(self: ParticleSystemNode, packet: Packet)
	...
	if instance.data then
		if instance.data.time then
			...
			instance.data.time.value = string.format("%d:%02d", displayH, m)
		end
		
		if instance.data.bpm then
			local bpmVal = mrandom(60, 170)
			instance.data.bpm.value = bpmVal
			newParticle.birthBpm = bpmVal
		end
...
local function spawnGroupPoints(self: ParticleSystemNode)
	if self.groupPointsCreated then return end
	self.groupPointsCreated = true

	local parts = self._particles
	local count = #parts
	local clusters: { [number]: ClusterStat } = {}
	...
	for i = 1, count do
		...
		if not p.isGroupPoint then
			...
			if not c then
				...
				-- Try to recover type from packet config
				if self.packets[cid] then
					c.type = self.packets[cid].type
				end
				clusters[cid] = c
			end
			c.sumX = c.sumX + p.x
	...
	-- 2. Spawn Group Points
	for cid, c in pairs(clusters) do
		if c.n > 0 then
			local avgX = c.sumX / c.n
			local avgY = c.sumY / c.n
			
			local gpX = self.groupPointX
			local gpY = avgY 
			print(gpY)
			
			-- Normalize size 2.0 to 3.0 based on actual cluster sizes
			local t = (c.n - globalMinN) / (globalMaxN - globalMinN)
			local scale = (2.0 + t) * self.sizeMultiplier
			local radius = BASE_RADIUS * scale
			
			local inputAb
			if c.type == 1 then inputAb = self.artboard1
			elseif c.type == 2 then inputAb = self.artboard2
			elseif c.type == 3 then inputAb = self.artboard3
			elseif c.type == 4 then inputAb = self.artboard4
			else inputAb = self.artboard5 end
			...
			if instance then
				...
				if instance.data then
					if instance.data.time then
						local function fmtT(m: number)
							...
							return string.format("%d:%02d", dispH, m % 60)
						end
					...
					if instance.data.active then
						-- Highlight group points
						instance.data.active.value = true
					end
				end
				...
				local gp: Particle = {
					...
					currentT = 0,
					colRadius = radius, -- Set initial colRadius
					nextInCell = nil,
					birthTime = 0,
					birthBpm = 0,
					isGroupPoint = true

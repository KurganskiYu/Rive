...
type ParticleSystemNode = {
	...
	zoomProperty: Property<boolean> | nil, -- Reference to VM property
...
local function init(self: ParticleSystemNode, context: Context): boolean
	...
	if vm then
  		...
  		self.zoomProperty = vm:getBoolean('zoom')
	end
...
local function relax(self: ParticleSystemNode, dt: number)
    ...
    if self.stats then
        if self.statsDelayTimer < 60 then
            self.statsDelayTimer = self.statsDelayTimer + 1
        end
    else
        self.statsDelayTimer = 0
    end

    local useStats = self.stats and (self.statsDelayTimer >= 60)
	...
	for i = #parts, 1, -1 do
		...
		if dx*dx + dy*dy <= p.radius * p.radius then
			...
			if p.isGroupPoint then
				...
				-- Update zoom: true if ANY group point is expanded, false otherwise
				local anyExpanded = false
				for j = 1, #parts do
					local gp = parts[j]
					if gp.isGroupPoint and gp.expanded then
						anyExpanded = true
						break
					end
				end
				
				if self.zoomProperty then
					self.zoomProperty.value = anyExpanded
				end
...
local function spawnSingleTimeMarker(self: ParticleSystemNode, data: PendingTimeMarker)
    ...
    if instance then
        ...
        if instance.data and instance.data.timeOff then
            instance.data.timeOff.value = self.stats
        end
...
local function advance(self: ParticleSystemNode, seconds: number): boolean
    ...
    -- Update time markers visibility based on stats mode
    local statsActive = self.stats
    if self.timeMarkers then
        for i = 1, #self.timeMarkers do
            local m = self.timeMarkers[i]
            if m.instance.data and m.instance.data.timeOff then
                 if m.instance.data.timeOff.value ~= statsActive then
                     m.instance.data.timeOff.value = statsActive
                 end
            end
        end
    end

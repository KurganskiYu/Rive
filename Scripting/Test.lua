...
local function dotGridGradient(
  ix: number,
  iy: number,
  dx: number,
  dy: number
...
function getOffset(
  ...
  if pathLength > 0 then
    ...
    if self.startFade > 0 then
      ...
      if distance < sf then
        fade = fade * ((distance / sf) ^ 2)
      end
    end
    -- End fade
    if self.endFade > 0 then
      local ef = self.endFade * 0.01 * pathLength
      if distance > (pathLength - ef) then
        fade = fade * (((pathLength - distance) / ef) ^ 2)
      end
    end
  end
...
function update(self: RippleEffect, path: PathData): PathData
  ...
  for i = 1, #path do
    ...
    elseif cmd.type == 'lineTo' then
      ...
      if currentSubIdx > 0 then
        subPathLengths[currentSubIdx] = subPathLengths[currentSubIdx] + d
      end
      sumTotal = sumTotal + d
      px, py = pt.x, pt.y
  ...
  if self.subdivision <= 0 then
    ...
    for i = 1, #path do
      ...
      elseif cmd.type == 'lineTo' then
        ...
        local offX, offY =
          self:getOffset(dist + len, currentPathLen, pt.x, pt.y, px2, py2)
      ...
      elseif cmd.type == 'cubicTo' then
        ...
        local o3x, o3y =
          self:getOffset(dist + len, currentPathLen, ep.x, ep.y, px2, py2)
      ...
      elseif cmd.type == 'quadTo' then
        ...
        local o1x, o1y =
          self:getOffset(dist + len * 0.5, currentPathLen, cp.x, cp.y, px2, py2)
        local o2x, o2y =
          self:getOffset(dist + len, currentPathLen, ep.x, ep.y, px2, py2)
  ...
  for i = 1, #path do
    ...
    elseif cmd.type == 'lineTo' then
      ...
      local dx, dy = pt.x - lastX, pt.y - lastY
      local len = length(dx, dy)
      local steps = math.min(math.ceil(len * density), 500)
      if steps < 1 then
        steps = 1
      end
      ...
      for s = 1, steps do
        ...
        local offX, offY =
          self:getOffset(dist + cl, currentPathLen, bx, by, px2, py2)
    ...
    elseif cmd.type == 'cubicTo' then
      ...
      local steps = math.min(math.ceil(approxLen * density), 500)
      if steps < 1 then
        steps = 1
      end
      ...
      for s = 1, steps do
        ...
        local dx = 3 * mt2 * (cp1.x - sx)
          + 6 * mt * t * (cp2.x - cp1.x)
          + 3 * t2 * (ep.x - cp2.x)
        local dy = 3 * mt2 * (cp1.y - sy)
          + 6 * mt * t * (cp2.y - cp1.y)
          + 3 * t2 * (ep.y - cp2.y)
        ...
        local px2, py2 = -ny, nx
        local offX, offY =
          self:getOffset(dist + approxLen * t, currentPathLen, x, y, px2, py2)
